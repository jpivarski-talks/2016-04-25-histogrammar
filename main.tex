...
\hfill ``fill rule:'' $f: \mathcal{D} \to \Bbb{R}$ \hspace{2.1 cm}
...
\end{lstlisting}
where {\tt \small q} is a function from $\mathcal{D} \to \Bbb{R}$ and {\tt \small cuts} are successively tighter thresholds; $N_{\mbox{\scriptsize cuts}} + 1$ histograms are created.
